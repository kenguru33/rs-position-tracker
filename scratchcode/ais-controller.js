...
module.exports = {
    ...
    getPositions(req,res) {
        ...
        ais.repository.getAisPositions(req.params.fromTime,req.params.toTime,req.params.mmsi)
            .then(aisData => {
                if(!aisData.length) {
                    let wideTimeSpan = moment(req.params.fromTime).subtract(15,'minutes').format("YYYY-MM-DDTHH:mm:ss");
                    console.log(`No positions found. Making timespan window wider. new from time: ${req.params.fromTime} -> ${wideTimeSpan}`);
                    ais.repository.getAisPositions(wideTimeSpan,req.params.toTime,req.params.mmsi)
                        .then(aisData=>{
                            res.send(aisData[aisData.length-1]);
                        })
                } else {
                    res.send(aisData);
                }
